# Adds a new member hopsital configuration to the updated config file (cfg_block_no_envelope_mod.json)
# Before executing this script:
#   1. Identity for the new HOSPITAL has been created
#   2. Execute the ./fetch-config-json.sh script
#   3. Setup the configtx.yaml file in ./config folder   
#      [e.g., refer to ../configs/orderer/hospital2/configtx.yaml in which Hospital2 hopsital is added]

cp ../configs/orderer/hospital2/configtx.yaml ./config

# CHANGE THESE BASED ON NEEDS
CHANNEL_ID="hospitalchannel"
NEW_ORG_NAME="Hospital2"
...
# 2. Add the Hospital2 Org to the (cfg_block_no_envelope_mod.json)
