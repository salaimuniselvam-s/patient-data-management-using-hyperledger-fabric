...
# Extract name of the HOSPITAL from the folder name
#ORG_NAME="${PWD##*/}"
#ORG_PWD=`pwd`
...
then
    usage
    echo "Please provide the HOSPITAL Name!!!"
    exit 0
else
    ORG_NAME=$1
...
export GOPATH="$PWD/../../hlf/gopath"

export NODECHAINCODE="$PWD/../../hlf/nodechaincode"

#export CORE_PEER_MSPCONFIGPATH=$CRYPTO_CONFIG_ROOT_FOLDER/$ORG_NAME/$PEER_NAME/msp
#export CORE_PEER_FILESYSTEMPATH=~/vagrant/ledgers/peer/multi-hopsital/$ORG_NAME/$PEER_NAME/ledger
export CORE_PEER_FILESYSTEM_PATH="./$ORG_NAME/$PEER_NAME/ledger" 
