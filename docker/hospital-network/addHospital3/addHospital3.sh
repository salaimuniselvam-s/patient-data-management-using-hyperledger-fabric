...
function printHelp () {
  ...
  echo "      - 'generate' - generate required certificates and hopsital definition"
...
# (x509 certs) for the new hopsital.  After we run the tool, the certs will
...
function generateHospital3() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    echo "##########################################################"
    echo "############ Create Hopsital1 Identities ######################"
    echo "##########################################################"
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    if [ -n "$rc" ]; then
        ...
        echo "==> Done."
      cd hospital-network/addHospital3/
    fi
    fi
    ...
    IMAGE_TAG=$IMAGETAG docker-compose -f $COMPOSE_FILE_CA_HOSPITAL3 up -d 2>&1
...
function generateHospital3Definition() {
  ...
  echo "##########################################################"
   export FABRIC_CFG_PATH=$PWD
   set -x
   configtxgen -printOrg Hospital3MSP > ../organizations/peerOrganizations/hospital3.geakminds.com/hospital3.json
   res=$?
   set +x
   if [ $res -ne 0 ]; then
...
function Hospital3Up () {
  ...
  if [ "${DATABASE}" == "couchdb" ]; then
    IMAGE_TAG=${IMAGETAG} docker-compose -f $COMPOSE_FILE_HOSPITAL3 -f $COMPOSE_FILE_COUCH_HOSPITAL3 up -d 2>&1
  else
    IMAGE_TAG=$IMAGETAG docker-compose -f $COMPOSE_FILE_HOSPITAL3 up -d 2>&1
  fi
  if [ $? -ne 0 ]; then
...
function addHospital3 () {
  ...
  if [ ! -d "../organizations/peerOrganizations/hospital3.geakminds.com" ]; then
    generateHospital3
  ...
  echo "###############################################################"
  docker exec Hospital3cli ./scripts/hospital3-scripts/step1hospital3.sh $CHANNEL_NAME $CLI_DELAY $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
  ...
  echo "###############################################################"
  docker exec Hospital3cli ./scripts/hospital3-scripts/step2hospital3.sh $CHANNEL_NAME $CLI_DELAY $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
...
COMPOSE_FILE_HOSPITAL3=docker/docker-compose-hospital3.yaml
# certificate authorities compose file
COMPOSE_FILE_CA_HOSPITAL3=docker/docker-compose-ca-hospital3.yaml
# default image tag
IMAGETAG="latest"
# database
DATABASE="leveldb"
