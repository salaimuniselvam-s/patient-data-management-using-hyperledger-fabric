...
class HospitalContract extends Contract {
  ...
  async CreatePatientRecord(ctx, userObj) {
    ...
    const bloodgroup = userObj.bloodgroup;
  ...
  // DoctorReadPatientRecord returns the Patient Record stored in the world state with given patientId  if the Patient given Authorization to the Doctor.
  async DoctorReadPatientRecord(ctx, userObj) {
    userObj = JSON.parse(userObj);
    const patientId = userObj.patientId;
    const doctorId = userObj.doctorId;
  ...
  // UpdatePatientRecordByDoctor updates an existing record in the world state with provided parameters if the Patient given Authorization to the Doctor.
  async UpdatePatientRecordByDoctor(ctx, userObj) {
    userObj = JSON.parse(userObj);
    const patientId = userObj.patientId;
    const diagnosis = userObj.diagnosis;
    const doctorId = userObj.doctorId;
  ...
  // DeletePatientRecord deletes a given record from the world state.
  async DeletePatientRecord(ctx, userObj) {
    userObj = JSON.parse(userObj);
    const patientId = userObj.patientId;
    const doctorId = userObj.doctorId;
    const recordJSON = await ctx.stub.getState(patientId);
  ...
  // PatientRecordExists returns true when record with given PatientId exists in world state.
  async PatientRecordExists(ctx, patientId) {
    const recordJSON = await ctx.stub.getState(patientId);
    return recordJSON && recordJSON.length > 0;
  }

  // TransferPatientRecord updates the DoctorId field of record with given id in the world state.
  async TransferPatientRecord(ctx, id, newDoctor) {
  ...
  // GetAllPatientRecords returns all records found in the world state.
  ...
  // GetPatientRecordHistory returns the history of a particular record
  async GetPatientRecordHistory(ctx, userObj) {
    userObj = JSON.parse(userObj);
  ...
  //  CheckAuthorization  Checks wherther the doctor is authrized to read/write Patient record
  ...
  async ReadPatientRecord(ctx, userObj) {
    userObj = JSON.parse(userObj);
    const patientId = userObj.patientId;
    const recordJSON = await ctx.stub.getState(patientId);
    if (!recordJSON || recordJSON.length === 0) {
