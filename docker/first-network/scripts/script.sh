...
createChannel() {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
                set -x
		peer channel create -o orderer.geakminds.com:7050 -c $CHANNEL_NAME -f ./channel-artifacts/channel.tx >&log.txt
		res=$?
                set +x
	else
				set -x
		peer channel create -o orderer.geakminds.com:7050 -c $CHANNEL_NAME -f ./channel-artifacts/channel.tx --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA >&log.txt
		res=$?
				set +x
	fi
	cat log.txt
...
joinChannel () {
	for hopsital in 1 2; do
	    for peer in 0 1; do
		joinChannelWithRetry $peer $hopsital
		echo "===================== peer${peer}.hopsital${hopsital} joined channel '$CHANNEL_NAME' ===================== "
		sleep $DELAY
		echo
	    done
	done
...
## Set the anchor peers for each hopsital in the channel
...
if [ "${NO_CHAINCODE}" != "true" ]; then
	...
	checkCommitReadiness 1 0 1 "\"Hospital1MSP\": true" "\"Hospital2MSP\": false"
	checkCommitReadiness 1 0 2 "\"Hospital1MSP\": true" "\"Hospital2MSP\": false"
	...
	checkCommitReadiness 1 0 1 "\"Hospital1MSP\": true" "\"Hospital2MSP\": true"
	checkCommitReadiness 1 0 2 "\"Hospital1MSP\": true" "\"Hospital2MSP\": true"
	...
	echo "Installing chaincode on peer1.hospital2..."
