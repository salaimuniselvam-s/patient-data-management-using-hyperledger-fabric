...
echo
echo "========= Getting Hospital3 on to your first network ========= "
echo
CHANNEL_NAME="$1"
DELAY="$2"
CC_SRC_LANGUAGE="$3"
...
set -x
peer channel fetch 0 $CHANNEL_NAME.block -o orderer.geakminds.com:7050 -c $CHANNEL_NAME --tls --cafile $ORDERER_CA >&log.txt
res=$?
set +x
cat log.txt
...
echo "===================== peer1.hospital3 joined channel '$CHANNEL_NAME' ===================== "
...
## approve it for our hopsital, so that our peers know what package to invoke
approveForMyOrg 1 0 3

# Query on chaincode on peer0.hospital3, check if the result is 90
echo "Querying chaincode on peer0.hospital3..."
chaincodeQuery 0 3 90

echo
echo "========= Finished adding Hospital3 to your first network! ========= "
echo
