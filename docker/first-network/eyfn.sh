...
function networkUp () {
  ...
  if [ ! -d "hospital3-artifacts/crypto-config" ]; then
    generateCerts
    generateChannelArtifacts
    createConfigTx
  fi
  # start hospital3 peers
...
# Generates Hospital3 certs using cryptogen tool
function generateCerts (){
  ...
  echo
  echo "###############################################################"
  echo "##### Generate Hospital3 certificates using cryptogen tool #########"
  echo "###############################################################"

  (cd hospital3-artifacts
   set -x
   cryptogen generate --config=./hospital3-crypto.yaml
   res=$?
   set +x
   if [ $res -ne 0 ]; then
...
function generateChannelArtifacts() {
  ...
  echo "##########################################################"
  echo "#########  Generating Hospital3 config material ###############"
  echo "##########################################################"
  (cd hospital3-artifacts
   export FABRIC_CFG_PATH=$PWD
   set -x
   configtxgen -printOrg Hospital3MSP > ../channel-artifacts/hospital3.json
   res=$?
   set +x
   if [ $res -ne 0 ]; then
  ...
  cp -r crypto-config/ordererOrganizations hospital3-artifacts/crypto-config/
  echo
}
...
#
COMPOSE_FILE_COUCH_ORG3=docker-compose-couch-hospital3.yaml
