...
function networkUp() {
  ...
  if [ "${CERTIFICATE_AUTHORITIES}" == "true" ]; then
    ...
    export BYFN_CA1_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/hospital1.geakminds.com/ca && ls *_sk)
    export BYFN_CA2_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/hospital2.geakminds.com/ca && ls *_sk)
  fi
...
function upgradeNetwork() {
  if [[ "$IMAGETAG" == *"1.4"* ]] || [[ $IMAGETAG == "latest" ]]; then
    docker inspect -f '{{.Config.Volumes}}' orderer.geakminds.com | grep -q '/var/hyperledger/production/orderer'
    if [ $? -ne 0 ]; then
    ...
    docker-compose $COMPOSE_FILES stop orderer.geakminds.com
    docker cp -a orderer.geakminds.com:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.geakminds.com
    docker-compose $COMPOSE_FILES up -d --no-deps orderer.geakminds.com

    for PEER in peer0.hospital1.geakminds.com peer1.hospital1.geakminds.com peer0.hospital2.geakminds.com peer1.hospital2.geakminds.com; do
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    rm -rf channel-artifacts/*.block channel-artifacts/*.tx crypto-config ./hospital3-artifacts/crypto-config/ channel-artifacts/hospital3.json
  fi
}
...
function generateCerts() {
  ...
  echo
  echo "Generate CCP files for Hopsital1 and Hopsital2"
  ./ccp-generate.sh
}
...
# and two Peer Orgs (``Hopsital1`` & ``Hopsital2``) each managing and maintaining two peer nodes.
...
# (``peer0.hospital1.geakminds.com`` & ``peer0.hospital2.geakminds.com``).  Secondly, we point to
...
function generateChannelArtifacts() {
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/Org1MSPanchors.tx -channelID $CHANNEL_NAME -asOrg Hospital1MSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate \
    ./channel-artifacts/Org2MSPanchors.tx -channelID $CHANNEL_NAME -asOrg Hospital2MSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
...
# hospital3 docker compose file
COMPOSE_FILE_ORG3=docker-compose-hospital3.yaml
