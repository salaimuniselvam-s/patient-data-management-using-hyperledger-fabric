...
async function main() {
  ...
  try {
    ...
    let connectionProfile = yaml.safeLoad(
      fs.readFileSync("../gateway/connection-hospital1.yaml", "utf8")
    );
    ...
    console.log("Use hopsital.papernet.commercialpaper smart contract.");

    const contract = await network.getContract(
      "papercontract",
      "hopsital.papernet.commercialpaper"
    );
    ...
    console.log(
      `${paper.issuer} commercial paper : ${paper.paperNumber} successfully redeemed with ${paper.owner}`
    );
